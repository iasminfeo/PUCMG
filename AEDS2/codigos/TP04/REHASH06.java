...
class Hash {
    ...
    public boolean pesquisar(Pokemon p) {
        comparacoes = 0; // Reseta contador de comparações
        boolean resp = false;
        int pos = calcularHash(p);
        comparacoes++;
        if (tabela[pos] == p) {
...
public class REHASH06 {
    public static void main(String[] args) {
        ...
        try {
            String logFileName = "854946_hashRehash.txt";
            long executionTime = endTime - startTime;
            int comparacoes = hashDiretaReHash.getComparacoes();

            try (BufferedWriter writer = new BufferedWriter(new FileWriter(logFileName))) {
                writer.write("854946\t" + executionTime + "\t" + comparacoes);
            }
        } catch (IOException e) {
