...
public class AVAV02 {
    public static void main(String[] args) {
        ...
        ArvoreArvore pkmns = new ArvoreArvore();
...
class ArvoreArvore {
    ...
    private boolean pesquisar(String x, NoArvore i) {
        boolean resp;

        if(i != null){
            resp = i.arvore.pesquisar(x);
    ...
    private NoArvore inserir(Pokemon x, NoArvore i) throws Exception {
        int resto = x.getCaptureRate() % 15;
        if (i == null) {
            throw new Exception("CaptureRate inexistente");
        } else if (resto == i.captureRate) {
            i.arvore.inserir(x);
        } else if (resto < i.captureRate) {
    ...
    private NoArvore inserir(int x, NoArvore i) throws Exception {
        if(i == null){
            i = new NoArvore(x);
            i.arvore = new ArvoreBinaria();
        }
        else if(x < i.captureRate){
            i.esq = inserir(x, i.esq);
        }
        else if(x > i.captureRate){
            i.dir = inserir(x, i.dir);
